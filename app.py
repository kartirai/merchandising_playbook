...
with tabs[0]:
    ...
    st.markdown("---")
    st.markdown('<div class="section-header">Google Trends — smartphone search interest in India (last 12 months)</div>', unsafe_allow_html=True)
        ...
        components.html("""
            ...
            <script type="text/javascript">
                trends.embed.renderExploreWidget(
                    "TIMESERIES",
                    {
                        "comparisonItem": [{"keyword": "smartphones", "geo": "IN", "time": "today 12-m"}],
                        "category": 0,
                        "property": ""
                    },
                    {
                        "exploreQuery": "geo=IN&q=smartphones&date=today 12-m",
...
with tabs[3]:
    ...
    # ── Top-level filters — drive everything below ──
    ...
    with f2:
        ...
        status_filter = st.multiselect(
            ...
            default=[s for s in ["Current series", "Latest launch"] if s in all_statuses],
            help="Filter applies to metric cards, charts, and both tables below"
        )
    with f3:
        gap_only = st.checkbox("CC gaps only", value=False, help="Show only products not listed on CC")

    # ── Apply filters once — reused everywhere ──
    brand_df = df[df["Brand"] == brand_filter].copy()
    filtered_df = brand_df[brand_df["Status"].isin(status_filter)] if status_filter else brand_df.copy()
    display_df = filtered_df[filtered_df["CC Status"] == "No"] if gap_only else filtered_df.copy()

    st.markdown("---")

    # ── Metric cards ──
    c1, c2, c3, c4 = st.columns(4)
    total_v  = len(filtered_df)
    gap_v    = len(filtered_df[filtered_df["CC Status"] == "No"])
    ok_v     = len(filtered_df[filtered_df["CC Status"] == "Yes"])
    top_v    = len(filtered_df[filtered_df["Top sellers"].astype(str).str.strip() == "Yes"])

    status_label = ", ".join(status_filter) if status_filter else "all statuses"
    with c1:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Total variants</div><div class="metric-val">{total_v}</div><div class="metric-sub">{status_label}</div></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Not on CC</div><div class="metric-val" style="color:#c0392b">{gap_v}</div><div class="metric-sub">ecommerce gap</div></div>', unsafe_allow_html=True)
    with c3:
    ...
    with c4:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Top sellers</div><div class="metric-val">{top_v}</div><div class="metric-sub">flagged in filtered set</div></div>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        if "Series" in filtered_df.columns:
            series_cc = filtered_df.groupby(["Series", "CC Status"])["Product Name"].count().reset_index()
            series_cc.columns = ["Series", "CC Status", "Count"]
            series_cc = series_cc[series_cc["Series"].notna() & (series_cc["Series"].astype(str) != "nan")]
            if not series_cc.empty:
                ...
                st.info("No series data for current selection.")

    with col2:
        st.markdown('<div class="section-header">Positioning breakdown (CC gap vs listed)</div>', unsafe_allow_html=True)
        pos_cc = filtered_df.groupby(["Positioning", "CC Status"])["Product Name"].count().reset_index()
        ...
        if not pos_cc.empty:
            ...
            st.info("No positioning data for current selection.")

    st.markdown("---")

    # ── Detail table ──
    st.markdown(f'<div class="section-header">CC gap detail — {len(display_df)} variants</div>', unsafe_allow_html=True)
    ...
    rc_cols = [c for c in rc_cols if c in display_df.columns]
    rc_display = display_df[rc_cols].copy()
    ...
    top_df = (
        filtered_df[["Product Name", "Price_clean", "Rating", "Volume", "Positioning", "CC Status", "Top sellers"]]
    ...
    if not top_df.empty:
        top_df["Top sellers"] = top_df["Top sellers"].apply(lambda x: "★" if str(x).strip() == "Yes" else "")
        ...
        top_df.columns = ["Product", "Price", "Rating", "Volume", "Segment", "CC Status", "Top seller"]
        st.dataframe(top_df.style.apply(highlight_cc, axis=1), use_container_width=True, hide_index=True)
    else:
        st.info(f"No products found for the current selection.")

...
with tabs[3]:
    ...
    c1, c2, c3, c4 = st.columns(4)
    total_v = len(brand_df)
    gap_v = len(brand_df[brand_df["CC Status"] == "No"])
    ok_v = len(brand_df[brand_df["CC Status"] == "Yes"])
    top_v = len(brand_df[brand_df["Top sellers"] == "Yes"])

    with c1:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Total variants</div><div class="metric-val">{total_v}</div><div class="metric-sub">in market</div></div>', unsafe_allow_html=True)
    with c2:
    ...
    if gap_only:
        show_df = brand_df[brand_df["CC Status"] == "No"]
    else:
        show_df = brand_df
    ...
    st.markdown("---")
    st.markdown('<div class="section-header">Top sellers with CC status</div>', unsafe_allow_html=True)
    top_df = brand_df[brand_df["Top sellers"] == "Yes"][
        ["Product Name", "Price_clean", "Rating", "Volume", "Positioning", "CC Status"]
    ...
    if not top_df.empty:
        ...
        top_df.columns = ["Product", "Price", "Rating", "Volume", "Segment", "CC Status"]
        st.dataframe(top_df.style.apply(highlight_cc, axis=1), use_container_width=True, hide_index=True)
    else:
        st.info(f"No top sellers flagged for {brand_filter} in the current data.")
